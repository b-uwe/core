...
_LOGGER.debug("Gig Radar module imported!")

# Very simple type...
type GigRadarConfigEntry = ConfigEntry
...
async def async_setup(hass: HomeAssistant, _config: ConfigType) -> bool:
    """Set up Gig Radar from YAML."""
    _LOGGER.debug("Setting up Gig Radar from configuration.yaml")
    ...
    if not existing_entries:
        _LOGGER.debug(
            "Creating initial Gig Radar Service to be managed from UI from there on"
        )
...
async def async_setup_entry(hass: HomeAssistant, entry: GigRadarConfigEntry) -> bool:
    """Set up Gig Radar from a config entry."""
...
async def async_unload_entry(hass: HomeAssistant, entry: GigRadarConfigEntry) -> bool:
